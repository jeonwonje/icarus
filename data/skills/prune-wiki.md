# Prune wiki — sweep this topic's `wiki/` for orphans, redundancy, and weird pages
...
The skill operates on **the current topic's folder** (your cwd). The weekly timer invokes you with cwd set to a synthetic cross-topic context — see "Cross-topic mode" below.
...
3. **Duplicates / near-duplicates** — pages whose subject overlaps another page in the same topic. Merge into the canonical page; redirect by deleting the loser and updating links + `index.md`.
...
## Cross-topic mode (weekly timer)

When the weekly-prune script invokes you, its cwd is `data/` (not a thread folder). In that case:

1. List `data/threads/*/` — each subdirectory is a topic.
2. For each topic, run the single-topic procedure above using `data/threads/<id>/wiki/`, `data/threads/<id>/index.md`, etc.
3. Surface ambiguous candidates per-topic into `data/threads/<id>/outbox/wiki-prune-<YYYY-MM-DD>.md` so each topic's owner sees only their own report.
4. Append to **each topic's** `log.md` (not a global one).
5. Reply with a one-paragraph summary including counts per topic.

## Output conventions

- Brand voice: no em dashes (`—` is fine in skill files but not in user-facing wiki / outbox content). Use `,`, `;`, or `-`.
- Dates: ISO 8601.
- Include counts in the log line, e.g. `2026-04-29 09:00 — pruned 7 orphans, 3 stubs; flagged 4 candidates [prune]`.
...
- Don't touch `data/CLAUDE.md`, `data/skills/`, or anything outside the topic's `wiki/` and `index.md`.
- Don't delete a topic's last remaining page just to clean up — leave a stub if the topic is still active.

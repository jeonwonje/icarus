...
- `/canvas` (and `scripts/canvas-sync.ts`) mirror NUS Canvas active-course files into `raw/canvas/<course>/`, plus announcements (markdown) and their accessible attachments into `raw/canvas/<course>/announcements/`, all **read-only** (chmod 0444 + a sandbox `--ro-bind` over `raw/canvas`). `/canvas` runs in the background and acks immediately (a full sync can be multi-GB). The sync runs in the bot process, not the agent, so `CANVAS_API_TOKEN` never reaches the sandboxed claude (it's also scrubbed from the agent's env). Config: `CANVAS_BASE_URL`, `CANVAS_API_TOKEN`, `CANVAS_COURSES`. Built in `src/canvas.ts`.

...
const ENV = 'ICARUS_CALENDAR_MCP';

function selftestStub(): McpStdioConfig {
  return { type: 'stdio', command: process.execPath, args: ['-e', 'process.exit(0)'] };
}

export function calendarConfig(input: { selftest: boolean; raw?: string }): McpStdioConfig {
  if (input.selftest) return selftestStub();
  const raw = input.raw ?? process.env.ICARUS_CALENDAR_MCP;
